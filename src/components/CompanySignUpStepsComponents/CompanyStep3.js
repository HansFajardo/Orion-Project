...
import FocustList from '../FocusList/FocusList';
import { Cell, Pie, PieChart } from 'recharts';

const COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#AF19FF', '#A7A9AB'];

export default function CompanyStep3({ onNext, onBack, selectedIndustries }) {
    const [selectedCategories, setSelectedCategories] = useState([]);
    const [showNotification, setShowNotification] = useState(false);

    const industries = selectedIndustries.map(item => item.serviceLine);

    const categories = FocustList.filter(category => industries.includes(category.industry));
        ...
        <div className="container rounded-3 border-3 shadow-lg p-5 w-50">
            ...
            <Row>
                ...
                <Col className='mt-4' md={6}>
                    ...
                    <div className='d-flex flex-row gap-3 justify-content-center'>
                        <button className='rounded-5 btn btn-outline-primary' onClick={removeAllServices}>
                            Remove All Focus
                        </button>
                    </div>
                </Col>
            ...
            {showNotification && (
                <Alert variant="danger" className="mt-3">
                    Please select at least one focus.
                </Alert>
            )}
